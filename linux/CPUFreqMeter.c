...
static void CPUFreqMeter_updateValues(Meter* this, char* buffer, size_t len) {
   this->values[0] = 0.0;
...
static void CPUFreqMeter_display(const Object* cast, RichString* out) {
   ...
   RichString_appendWide(out, freqColor, buffer);
   RichString_appendWide(out, CRT_colors[METER_TEXT], " MHz");
}
