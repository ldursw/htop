...
static void CPUTempMeter_updateValues(Meter* this, char* buffer, size_t len) {
   this->values[0] = 0.0;
...
static void CPUTempMeter_display(const Object* cast, RichString* out) {
   ...
   RichString_appendWide(out, tempColor, buffer);
   RichString_appendWide(out, CRT_colors[METER_TEXT], "\xc2\xb0" "C");
}
